...
async function scrapeScheduleData() {
  ...
  try {
    ...
    const scheduleHTML = await fetchHTML(
      "https://nomads.magneticfields.in/schedule",
    );

    // Look for embedded JSON data in script tags
    const jsonDataMatch = scheduleHTML.match(
...
async function main() {
  ...
  try {
    ...
    fs.writeFileSync("scraped-schedule.json", JSON.stringify(output, null, 2));
    ...
    console.log(
      `   - Total events: ${formatted.schedule.fri.length + formatted.schedule.sat.length + formatted.schedule.sun.length}`,
    );
    ...
    console.error(
      "   You may need to manually inspect the site and update this script.",
    );
    process.exit(1);
  }
}
